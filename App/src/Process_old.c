...
{ 
  ...
  a_Avg = Sample/2;   b_Avg = Sample/2;  //****
  a_Ssq = Sample/2;   b_Ssq = Sample/2;  //****    
  ...
  if(Interlace == 0){                           // 独立采样模式
    ...
    for(i=0; i <Sample; i++){
      ...
      else if((__Get(FIFO_EMPTY)==0)&&(i == JumpCnt)&&(_Status == RUN)){ //JumpCnt
        JumpCnt++;
    ...
    if(_1_source == HIDE)  a_Avg = _1_posi*Sample;   // A通道合并到B通道时      
    if(_2_source == HIDE)  b_Avg = _2_posi*Sample;   // B通道合并到A通道时
  }
