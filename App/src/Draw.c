...
u16 SpecRow = 0;
//u8 colors[400];
const u16 colors[200] = {32768,32768,32768,32768,34816,34816,34816,36864,36864,36864,38912,38912,
	38912,40960,40960,40960,43008,43008,43008,43008,45056,45056,45056,47104,47104,47104,49152,49152,
	49152,51200,51200,51200,53248,53248,53248,55296,55296,55296,55296,57344,57344,57344,59392,59392,
	59392,61440,61440,61440,63488,63488,63488,63520,63552,63584,63616,63680,63712,63744,63776,63840,
	63872,63904,63968,64000,64032,64064,64128,64160,64192,64224,64288,64320,64352,64416,64448,64480,
	64512,64576,64608,64640,64704,64736,64768,64800,64864,64896,64928,64960,65024,65056,65088,65152,
	65184,65216,65248,65312,65344,65376,65408,65472,65504,65504,65505,65505,65506,65507,65507,65508,
	65508,65509,65510,65510,65511,65512,65512,65513,65514,65514,65515,65515,65516,65517,65517,65518,
	65519,65519,65520,65521,65521,65522,65523,65523,65524,65524,65525,65526,65526,65527,65528,65528,
	65529,65530,65530,65531,65531,65532,65533,65533,65534,65535,65535,65503,63423,63391,61279,59199,
	59167,57087,54975,54943,52863,50751,50719,48639,48607,46495,44415,44383,42303,40191,40159,38079,
	35967,35935,33855,33823,31711,29631,29599,27519,25407,25375,23295,21183,21151,19071,19039,16927,
	14847,14815,12703,10623,10591,8511,6399,6367,4287,4255,2143,63};

u16 RowMem, RowMemPeak;

uc8  Mark_TAB[6][7] ={{0x00,0x00,0x42,0xFE,0x02,0x00,0x00},    // Mark 1
	...
	{0x00,0x00,0x00,0x00,0x00,0x00,0x00}};   // Clear Mark
...
void Draw_Meter_Mark(uc16 col)						
{ 
	u16 i, j, Tmp;

	for(i = 300; i <= 310; ++i){
		if(i < 300+7){
				Tmp = Mark_TAB[5][i-300];
				__Point_SCR(i,     99 +(MIN_Y-4));
			for(j = 0; j < 8; ++j){
				if(Tmp & 1) __LCD_SetPixl(Color[SCRN]);
				else        __LCD_SetPixl(col);
				Tmp >>= 1;
			}
			__LCD_SetPixl(col);
		} else {
			__Point_SCR(i,     99 +(MIN_Y-10)+ (i-300));   
			for(j=99+(MIN_Y-10)+(i-300);j<(99+(MIN_Y+11)-(i-300));++j) 
			__LCD_SetPixl(col); 
		}
	}
}
...
Print_Str2: 
*******************************************************************************/
void Print_Str2(u16 Type, u8 Mode, char *s)
{ 
	signed short i, j, b; 
	u8 xm, ym;
	u16 x0 = 11;
	
	__LCD_Set_Block(x0, 300, 60, 60+109);

	for (j=0; j<110;++j){ 
		if(Mode == 0) __LCD_SetPixl(Color[Type & 0x0F]);   //Normal replace Display
		else          __LCD_SetPixl(Color[Type >> 0x8]);   //Inverse replace Display
	}
	x0+=5;                            // a string to add a blank row
	while (*s!=0) {
		for(i=0;i<8;++i){
			if((*s==0x20)||(*s==0x21)) b = 0x0000;
			else                       b = Get_TAB_8x11(*s, i);
			...
			for(xm=0;xm<5;++xm){
				for(j=0;j<11;++j){
					if((b << j)& 0x400) {
						if(Mode == 0) for(ym=0;ym<10;++ym) __LCD_SetPixl(Color[Type >> 0x8]);
						else          for(ym=0;ym<10;++ym) __LCD_SetPixl(Color[Type & 0x0F]);
					} else {
						if(Mode == 0) for(ym=0;ym<10;++ym) __LCD_SetPixl(Color[Type & 0x0F]);
						else          for(ym=0;ym<10;++ym) __LCD_SetPixl(Color[Type >> 0x8]);
					}
				}
			}
		}
		if(*s==0x21) x0 +=4*5;           // display position horizontally +4 ("!" character - shift by 1/2 a character)
		else  x0 += 8*5;                 // display position horizontally +8
		++s;                           // string pointer +1
	}
	__LCD_Set_Block(LCD_X1,LCD_X2,LCD_Y1,LCD_Y2);  // restore the full-size window
	Draw_Meter_Mark(Color[Type >> 0x8]);
}
...
void Draw_Row(u16 Row)
{
	...
	case SCAN:
		...
		} else if ((_4_source!=BIG_M)) {
			Draw_Oscill_Grid(Row, LCD_NextBuffer);
			if ((Row > MIN_X) && (Row < MAX_X)) {
				Draw_Row_Oscill(Row, LCD_ActiveBuffer); //oscilloscope
			}
		}
		break;
	case X_Y:
